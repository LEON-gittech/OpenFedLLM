    ...
    for client in range(fed_args.num_clients):
        ...
        for epoch in range(script_args.self_reward_epochs):
            ...
            if script_args.use_vllm:
                model_cache_dir = "/mnt/bn/merlin-datavolume-tsy/leon/checkpoints/self-rewarding/vllm_model_cache"
                model.save_pretrained_merged(f"{model_cache_dir}", tokenizer, save_method = "merged_16bit",)
                model = model.cpu()
                model = LLM(model=model_cache_dir, tensor_parallel_size=1, dtype=torch.bfloat16, trust_remote_code=True, 
                    enable_lora=False, max_model_len=2048, gpu_memory_utilization=0.8)
                model.pad_token = tokenizer.eos_token
            elif script_args.unsloth:
            ...
            if epoch !=0 and script_args.rank_net: # 生成ref_model的scores
                ...
                ranknet_loss = 10 * compute_ranknet_loss(output_path, score_cache_path)
                optimizer.zero_grad()
                ranknet_loss.backward()
                optimizer.step()
